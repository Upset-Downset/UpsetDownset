...
def outcome_prediction(game, alpha_or_apprentice, look_ahead):
    ...
    look_ahead : int (nonnegative), optional
        the number of iterations of  MCTS to be performed for each turn. 
        The default is 800.
    ...
    net = utils.load_model(alpha_or_apprentice, device)
    ...
    # get outcomes
    up_start_out = outcome_play(
        up_start, 
        net, 
        device, 
        search_iters=look_ahead)
    down_start_out = outcome_play(
        down_start, 
        net, 
        device,
        search_iters=look_ahead)
