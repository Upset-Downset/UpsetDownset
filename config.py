...
MAX_NODES = 10    #max number of nodes ina game playable by the agent
...
RGB_DIST = (0.25, 0.75)    #distribution of game node colorings in self-play/eval : (RGB, all green)
...
DIRICHLET_ALPHA = 2.25  #dirichlet distribution parameter
...
#TRAINING PARAMETERS
GAMES_TO_TRAIN = 10000   #how long to wait for replay buffer to fill during training (seconds)
MAX_REPLAY_BUFFER = 1000000    #max length of training replay buffer
BATCH_SIZE = 4    #number of training examples per epoch (2)
NUM_SYMMETRIES = 8    #number of symmetries per training example per epoch (16)
LEARN_RATE = 0.001    #sgd learning rate
MOMENTUM = 0.9    #sgd momentum
REGULAR = 0.0001   # L2 weight regularization parameter for sgd

#EVALUATION PARAMETERS
ASYNC_EVALUATIONS = 2    #the number of parallel evaluation processes
PLAYS_PER_EVAL = 100   #number of games per evaluation processs
NUM_EVAL_PLAYS = ASYNC_EVALUATIONS * PLAYS_PER_EVAL    #total evaluation games played
EPOCHS_TO_EVAL = 250000   #number of training epochs between evaluations
EVAL_PLAY_MARKOV_EXP = 2    #exponent for markov process in evaluation game generation
EVAL_PLAY_SEARCH_ITERS = 600      #number of mcts search iterations in evaluation play (100)
