...
def current_alpha_iteration():
    ''' Returns the index of the most recent iteration of the alpha_model.

    Returns
    -------
    cur_alpha_idx : str
        the index of the most recent iteration of the alpha_model. I.e., 
        searches the directory "./model_data/alpha_data" for the most 
        recent additionn and takes the index.
        
    ** Basically the same function as 'last_training_iteration()', but searches 
    a differnt folder: two functions makes for better readability.
    
    '''
    path_to_alpha_models = os.path.join('./model_data/alpha_data')
    cur_alpha_model = path_to_latest_data(
        path_to_alpha_models).split('/')[-1]
    cur_alpha_idx = cur_alpha_model.split('_')[0]
    
    return cur_alpha_idx



def load_apprentice_model(apprentice_net, device):
    # get the current apprentice models index for loading paramater
    cur_idx = last_training_iteration()
          
    # if GPU is available, put model on GPU. (device index 0 by default)
    if device == 'cuda':
        apprentice_net.load_state_dict(
            torch.load(
                './model_data/apprentice_data/'+cur_idx+'_apprentice_net.pt', 
                map_location='cuda:0'))
        apprentice_net.to(device)
    # no GPU...
    else:
        apprentice_net.load_state_dict(
            torch.load(
...
def pickle_evaluation_data(data, proc_id, play_idx, train_iter):
    ''' Pickles gameplay data from evaluation-play. 

    Parameters
    ----------
    folder : str
        name of the folder we want to pickle to: evaluation_data.
    data : list
        data we want to pickle. The data for each game is stored as a 
        list of length 2:
            - model that moved first, 'alpha' or 'apprentice'
            - list of states encountered, including the terminal state.
    play_idx: int (nonnegative)
        the index of the game being played.
    proc_id : int (nonnegative)
        the process id in multiprocess
    train_iter : int (nonnegative)
        the current iteration of the main training pip
    ...
    '''
    path = os.path.join(
        './evaluation_data/iter_%d/evaluation_iter%d_proc%i_game_%i_%s' % 
        (train_iter,
         train_iter,
         proc_id, 
         play_idx,
...
def load_evaluation_data(train_iter):
    ''' loads all pickled evaluation-play data for itertion 'index'.
    ...
    -------
    eval_data : list
        each element in the list corresponds to a single game of 
        evaluation play. The data for each game is stored as a list 
        of length three:
            - model that moved first, 'alpha' or 'apprentice'
            - player which moved first, 'Up' or 'Down'
            - list of states encountered, including the terminal state.
    ...
    '''
    eval_data = []
    path = os.path.join("./evaluation_data/iter_%d" % train_iter)
    for filename in os.listdir(path):
        full_filename = os.path.join(path, filename)
        file  = open(full_filename, 'rb')
        data = pickle.load(file)
